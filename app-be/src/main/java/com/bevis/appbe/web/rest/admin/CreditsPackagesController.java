...
@RequestMapping("/api")
public class CreditsPackagesController {

    private final CreditsPackagesManagementService creditsPackagesManagementService;

    @Secured(ADMIN)
    @ApiPageable
    @GetMapping("admin/credits-packages")
    DataResponse<CreditsPackage> findAll(Pageable pageable) {
        log.debug("REST to load credits-packages...");
        Page<CreditsPackage> page = creditsPackagesManagementService.findAll(pageable);
